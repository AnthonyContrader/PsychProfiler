...
export abstract class AbstractService<DTO> implements Service<DTO> {
    ...
    getAll(): Observable<DTO[]>{
        return this.http.get<DTO[]>('http://localhost:8080/services/'+this.micro+'/api/'+this.type + 's', {
          headers: {
    ...
    delete(id: number): Observable<any> {
        return this.http.delete('http://localhost:'+ this.port +'/services/'+this.micro+'/api/'+ this.type + 's/' + id, {
            headers: {
                Authorization: this.auth()
            }
          });
    ...
    insert(dto: DTO): Observable<any> {
        return this.http.post('http://localhost:'+ this.port +'/services/'+this.micro+'/api/'+ this.type + 's', dto, {
            headers: {
                Authorization: this.auth()
            }
          });
