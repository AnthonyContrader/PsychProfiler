...
describe('Job e2e test', () => {
  ...
  it('should load Jobs', async () => {
    ...
    expect(await jobComponentsPage.getTitle()).to.eq('gatewayApp.micro1Job.home.title');
  });

  it('should load create Job page', async () => {
    await jobComponentsPage.clickOnCreateButton();
    jobUpdatePage = new JobUpdatePage();
    expect(await jobUpdatePage.getPageTitle()).to.eq('gatewayApp.micro1Job.home.createOrEditLabel');
  ...
  it('should create and save Jobs', async () => {
    ...
    await jobComponentsPage.clickOnCreateButton();
    await promise.all([jobUpdatePage.setRoleInput('role'), jobUpdatePage.setDescriptionInput('description')]);
    expect(await jobUpdatePage.getRoleInput()).to.eq('role', 'Expected Role value to be equals to role');
  ...
  it('should delete last Job', async () => {
    ...
    expect(await jobDeleteDialog.getDialogTitle()).to.eq('gatewayApp.micro1Job.delete.question');
